...
class TT_Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = tt_linearV2.tt_LinearV2(np.array([2, 2, 7, 4, 7]), np.array([8, 16, 16, 16, 8]), np.array([1, 4, 4, 4, 4, 1]))
        self.fc2 = tt_linearV2.tt_LinearV2(np.array([8, 16, 16, 16, 8]), np.array([5, 2, 1, 1, 1]), np.array([1, 4, 4, 4, 4, 1]))
        self.bn1 = nn.BatchNorm1d(262144)
...
class Usual_Net(nn.Module):
    def __init__(self):
        ...
        self.bn1 = nn.BatchNorm1d(262144)
        self.bn2 = nn.BatchNorm1d(10)
        self.fc1 = nn.Linear(28 * 28, 262144)
        self.fc2 = nn.Linear(262144, 10)
        ...
        for phase in ['train', 'test']:
            ...
            for inputs, labels in cur_set:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    outputs = model(inputs.reshape(-1, 28*28))
...
if __name__ == '__main__':
    ...
    optimizer = optim.SGD(tt_model.parameters(), lr=0.001, momentum=0.8)
    exp_lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
    tt_model = train_model(tt_model, criterion, optimizer, None, num_epochs=25)
